...
const std::vector<Patchwork::Filter> Mixture::filterCacheObj()
{
	return filterCache_;
}
					   ...
					   vector<vector<vector<Model::Positions> > > * positions) const
{
	...
	cout << "....called once ...... Inside Mixture-convolve, 1st step, model size" << nbModels << " pyramid levels " << pyramid.levels().size() << endl;
...
void Mixture::convolve(const HOGPyramid & pyramid,
					   ...
					   vector<vector<vector<Model::Positions> > > * positions) const
{
	cout << "....called once ...... Inside Mixture-convolve2, pyramid to patchwork, patchwork convolve," << endl;
	if (empty() || pyramid.empty()) {
		scores.clear();
...
{
	cout << "Inside Mixture-cacheFilters, calls TransformFilter" << endl;
...
{
	cout << "Inside Mixture-iStream" << endl;
	int nbModels;
	is >> nbModels;
	
	cout << " Mixture-iStream " << nbModels << endl;
